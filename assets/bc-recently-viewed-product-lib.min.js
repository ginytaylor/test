...
BCRecentProduct.prototype.saveRecentProductInfo = function() {
    var self = this;
    if (self.settings.isProductPage && typeof(Storage) !== 'undefined') {
        if (localStorage.getItem(self.settings.storageId) != null) {
            var currentRecentProductData = JSON.parse(localStorage.getItem(self.settings.storageId));
            if (window.bcOtpProductJsonData != '') {
                var productJsonData = window.bcOtpProductJsonData;
              

                var findIndex = currentRecentProductData.products.findIndex(function(product) {
                  if(product != null){

					return product.id == productJsonData.products[0].id;
                  }
                });
                if (findIndex == -1) {
                    if (currentRecentProductData.products.length >= self.settings.limit) {
                        currentRecentProductData.products.shift();
                    }
                    currentRecentProductData.products.push(productJsonData.products[0]);
                    localStorage.setItem(self.settings.storageId, JSON.stringify(currentRecentProductData));
                }
            }
        } else {
            if (typeof window.bcOtpProductJsonData !== 'undefined') {
                localStorage.setItem(self.settings.storageId, JSON.stringify(window.bcOtpProductJsonData));
            }
        }
    } else {
        console.info('BC: This browser does not support Storage')
    }
};
// SHOW PRODUCT FUNCTION
BCRecentProduct.prototype.buildRecentProductsHtml = function() {
    var self = this;
 	
    if (self.checkShowRecentProducts() && localStorage.getItem(self.settings.storageId) !== null && jQ(self.selector.recentProductSliderSelector).length > 0) {
      	var currentRecentProductList = JSON.parse(localStorage.getItem(self.settings.storageId));
        currentRecentProductList.products.reverse();
        var recentlySelector = jQ(self.selector.recentProductSliderSelector);
        recentlySelector.empty();
        var lengthRecentlyProduct = Math.min(currentRecentProductList.products.length, self.settings.limit);
        for (var i = 0; i < lengthRecentlyProduct; i++) {
            var product = currentRecentProductList.products[i];
          	if(product != null){
              if(product.id != bcRvParams.settings.productId){
                if (typeof bcsffilter !== 'undefined') {
                    var itemHtml = bcsffilter.buildProductGridItem(product, i + 1);

                    itemHtml += '<script id="ProductJson-bc-sf-recent-products-wrapper-'+ product.id +'" class="bc-sf-filter-product-script bold-product-json" data-id="bc-sf-recent-products-wrapper-'+ product.id +'" type="application/json">'+ JSON.stringify(product) +'</script>';
                    recentlySelector.append(itemHtml);
                } else {
                    recentlySelector.append(self.buildRecentProductItem(product));
                }
              }
            }
        }
        jQ(self.selector.recentProductWrapperSelector).prepend(self.template.headingTemplate.replace(/{{title}}/g, self.translation.recentProductHeading));
        if (self.settings.enableSlider) {
            self.recentProductInitCarousel(self.selector.recentProductSliderSelector, self.settings.recentProductSliderRange, true, true);
        }
...
// OwlSlider
BCRecentProduct.prototype.recentProductInitCarousel = function(selector, items, nav, loop) {
	var r_swiper = new Swiper('.productsSwiper', {
          loop: false,
       //   allowTouchMove: false,
          updateOnWindowResize: true,
          navigation: {
            nextEl: '.bc-sf-recent-products-wrapper .swiper-button-nexts',
            prevEl: '.bc-sf-recent-products-wrapper .swiper-button-prevs',
          },
            breakpoints: {
...
BCRecentProduct.prototype.init = function(bcSfFilterParams, params) {
    var self = this;
  self.rvPrepareParams(bcSfFilterParams, params);
    if (self.settings.recentlyViewedEnable) {
        self.saveRecentProductInfo();
        self.buildRecentProductsHtml();
    }else{
        self.removeRecentlyViewedData();
    }
  if($('.bc-sf-widget-products').html() == ''){
    
    // Find the container that will hold the recommendations
      var list = document.querySelector(".bc-sf-recent-products");
      ...
      request.open(
        "GET",
        baseUrl + ".json?product_id=" + productId + "&limit=36"
      );
      ...
      request.onload = function() {
        ...
        for (var i = 0; i < products.length ; i++) {
            ...
            product.images_info = product.media;
            if(product.id != bcRvParams.settings.productId){
              if (typeof bcsffilter !== 'undefined') {
                  var itemHtml = bcsffilter.buildProductGridItem(product, i );
                  itemHtml += '<script id="ProductJson-bc-sf-recent-products-wrapper-'+ product.id +'" class="bc-sf-filter-product-script bold-product-json" data-id="bc-sf-recent-products-wrapper-'+ product.id +'" type="application/json">'+ JSON.stringify(product) +'</script>';
                  listProductHtml += itemHtml;
              } else {
                  listProductHtml += self.buildRecentProductItem(product);
              }
            }
        }
       
        
        list.innerHTML = listProductHtml;
        self.recentProductInitCarousel('','','','');
      };
      ...
      function hideRecommendationsSection() {
        ...
        $('.bc-sf-widget-slider-wrapper').hide();
      }
  }
  var sections = new theme.Sections();
  sections.register('product-recommendations', theme.Product);
